...
impl LlmProvider for AnthropicProvider {
    async fn chat(&self, req: ChatRequest) -> Result<ChatResponse, ModelError> {
        ...
        let messages: Vec<_> = req.messages.iter().filter(|m| m.role != Role::System)
            .map(|m| serde_json::json!({"role": role_to(m.role), "content": m.content})).collect();
        ...
        let resp = self.client.post(&url).header("x-api-key", &self.api_key).header("anthropic-version", "2023-06-01")
                                    ...
                                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(data) {
                                        if val["type"] == "content_block_delta" {
                                            if let Some(text) = val["delta"]["text"].as_str() {
                                                let _ = tx.send(Ok(StreamChunk { content: text.to_string(), finish_reason: None }));
                                            }
                                        }
                                        if val["type"] == "message_stop" {
                                            let _ = tx.send(Ok(StreamChunk { content: String::new(), finish_reason: Some("stop".into()) }));
                                        }
                                    }
                                }
                            }
