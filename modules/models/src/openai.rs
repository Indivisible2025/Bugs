...
impl OpenAiProvider {
    pub fn new(name: impl Into<String>, base_url: impl Into<String>, api_key: impl Into<String>) -> Self {
        Self { name: name.into(), base_url: base_url.into(), api_key: api_key.into(), client: reqwest::Client::new() }
    }
    ...
    fn extract_reasoning(data: &serde_json::Value) -> Option<String> {
        data["choices"][0]["delta"]["reasoning_content"].as_str().map(|s| s.to_string())
            .or_else(|| data["choices"][0]["message"]["reasoning_content"].as_str().map(|s| s.to_string()))
    }
}
...
impl LlmProvider for OpenAiProvider {
    async fn chat(&self, req: ChatRequest) -> Result<ChatResponse, ModelError> {
        ...
        if !resp.status().is_success() {
            return Err(ModelError::ApiError(format!("HTTP {}: {}", resp.status(), resp.text().await.unwrap_or_default())));
        }
                    ...
                    Ok(bytes) => {
                        ...
                        for line in text.lines() {
                            ...
                            if let Ok(data) = serde_json::from_str::<serde_json::Value>(json_str) {
                                ...
                                let reasoning = Self::extract_reasoning(&data).unwrap_or_default();
                                let finish = data["choices"][0]["finish_reason"].as_str().map(|s| s.to_string());
                                let combined = if !reasoning.is_empty() { format!("[思考]\n{}\n[回答]\n{}", reasoning, content) } else { content };
                                let _ = tx.send(Ok(StreamChunk { content: combined, finish_reason: finish }));
                            }
                            }
                        }
                    }
    ...
    fn supports(&self, _model: &str) -> bool { true }
