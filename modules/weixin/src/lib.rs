...
impl WeixinModule {
    ...
    pub async fn bind(&mut self) -> Result<(), ModuleError> {
        ...
        // 启动回调服务器等待扫码结果
        let token = server::wait_for_bind(self.port)
            .await
...
impl BugModule for WeixinModule {
    ...
    fn description(&self) -> &str {
        "通过微信官方 API，扫码绑定个人微信，接收消息并交由 Overmind 回复"
    }
    ...
    fn capabilities(&self) -> Vec<ModuleCapability> {
        vec![ModuleCapability::Channel {
            name: "wechat-personal".into(),
        }]
    }
    fn permissions(&self) -> Vec<ModulePermission> {
        vec![
            ModulePermission::NetworkOutbound,
            ModulePermission::AccessMemory,
        ]
    }
}
