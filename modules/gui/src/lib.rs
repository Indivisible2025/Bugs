...
use eframe::egui::{
    self, vec2, Align, CentralPanel, Color32, Key, Layout, ScrollArea, SidePanel, TextEdit,
...
impl Default for BugsGui {
    fn default() -> Self {
        let mut g = Self {
            input: String::new(),
            messages: vec![("system".into(), "🧠 Overmind 已就绪".into())],
            panel: ActivePanel::Chat,
            scenes: vec!["general".into()],
            current_scene: "general".into(),
            trust_pending: Vec::new(),
            status_text: "获取中...".into(),
        };
        g.refresh_data();
        g
    }
...
impl eframe::App for BugsGui {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut Frame) {
        ...
        TopBottomPanel::top("tabs").show(ctx, |ui| {
            ui.horizontal(|ui| {
                if ui
                    .selectable_label(matches!(self.panel, ActivePanel::Chat), "💬 对话")
                    .clicked()
                {
                ...
                if ui
                    .selectable_label(matches!(self.panel, ActivePanel::Scenes), "📋 场景")
                    .clicked()
                {
                    self.panel = ActivePanel::Scenes;
                    self.refresh_data();
                }
                if ui
                    .selectable_label(matches!(self.panel, ActivePanel::Trust), "🔐 信任")
                    .clicked()
                {
                    self.panel = ActivePanel::Trust;
                    self.refresh_data();
                }
                if ui
                    .selectable_label(matches!(self.panel, ActivePanel::Status), "📊 状态")
                    .clicked()
                {
...
impl BugsGui {
        ...
        CentralPanel::default().show(ctx, |ui| {
                ...
                .show(ui, |ui| {
                    for (role, content) in &self.messages {
                        let color = match role.as_str() {
                            "user" => Color32::LIGHT_GREEN,
                            "assistant" => Color32::LIGHT_YELLOW,
                            _ => Color32::GRAY,
                        };
            ...
            ui.horizontal(|ui| {
                ...
                if r.lost_focus() && ui.input(|i| i.key_pressed(Key::Enter)) {
                    ...
                    if !msg.is_empty() {
                        self.messages.push(("user".into(), msg));
                    }
                }
            });
        });
    ...
    fn draw_trust(&mut self, ctx: &egui::Context) {
        CentralPanel::default().show(ctx, |ui| {
            ...
            ui.separator();
            if self.trust_pending.is_empty() {
                ui.label("✅ 无待确认记忆");
            }
            for m in &self.trust_pending {
...
pub async fn start() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eframe::run_native(
        "Bugs Overmind",
        options,
        Box::new(|_cc| Ok(Box::new(BugsGui::default()))),
    )?;
    Ok(())
}
