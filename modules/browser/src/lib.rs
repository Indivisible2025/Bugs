...
const TIMEZONES: &[&str] = &["Asia/Shanghai", "America/New_York", "Europe/London", "Asia/Tokyo"];
...
impl BrowserModule {
    ...
    fn get_chrome_path(&self) -> Result<Option<String>, ModuleError> {
        if let Ok(p) = std::env::var("CHROME_PATH") { return Ok(Some(p)); }
        if let Some(ref p) = *self.chromium_path.lock() { return Ok(Some(p.display().to_string())); }
    ...
    pub fn status(&self) -> String {
        ...
        format!("Headless: {}/{} | Headful: {}/{}", h.contexts.len(), h.max_size, f.contexts.len(), f.max_size)
    }

    /// Headless 抓取
    pub fn fetch(&self, url: &str, timeout_secs: u64) -> Result<String, ModuleError> {
        let chrome_path = self.get_chrome_path()?;
        self.manager.headless_pool.lock().execute(url, timeout_secs, true, &chrome_path)
    }

    /// Headful 模式抓取
    pub fn fetch_headful(&self, url: &str) -> Result<String, ModuleError> {
        let chrome_path = self.get_chrome_path()?;
        self.manager.headful_pool.lock().execute(url, 60, false, &chrome_path)
    }
    ...
    pub fn search_with(&self, engine: &str, query: &str) -> Result<String, ModuleError> {
        let url = self.search_registry.read().build_url(engine, query)
    ...
    pub fn add_engine(&self, id: String, name: String, url_template: String) {
        self.search_registry.write().register(id, search::SearchEngine {
            name, url_template,
            result_extract_js: "",
        });
    }
...
impl ContextPool {
    fn new(max_size: usize, max_tabs: usize) -> Self {
        Self { contexts: vec![], max_size, max_tabs_per_context: max_tabs,
            idle_dormant: Duration::from_secs(30), idle_destroy: Duration::from_secs(300),
            recycle_interval: 50 }
    }

    fn get_or_create(&mut self, chrome_path: &Option<String>, headless: bool) -> Result<(usize, Arc<Browser>), ModuleError> {
        // 找空闲 Context
        ...
        if self.contexts.len() < self.max_size {
            ...
            let entry = ContextEntry { browser: browser.clone(), tabs_used: 0, fingerprint: fp,
                created_at: Instant::now(), last_used: Instant::now(), search_count: 0 };
    ...
    fn execute(&mut self, url: &str, _timeout_secs: u64, headless: bool, chrome_path: &Option<String>) -> Result<String, ModuleError> {
        let (idx, browser) = self.get_or_create(chrome_path, headless)?;
        let tab = browser.new_tab().map_err(|e| ModuleError::Runtime(format!("tab: {e}")))?;
        ...
        let result = (|| -> Result<String, ModuleError> {
            ...
            // 实际导航
            tab.navigate_to(url).map_err(|e| ModuleError::Runtime(format!("nav: {e}")))?;
            tab.wait_until_navigated().map_err(|e| ModuleError::Runtime(format!("load: {e}")))?;
    ...
    fn reap_idle(&mut self) {
        let now = Instant::now();
        self.contexts.retain(|e| now.duration_since(e.last_used) <= self.idle_destroy);
    }
}

// ── 借鉴 Chromium LaunchOptions ──

fn launch_browser(chrome_path: &Option<String>, headless: bool, fp: &BrowserFingerprint) -> Result<Arc<Browser>, ModuleError> {
    let ua = OsString::from(format!("--user-agent={}", fp.user_agent));
    let size = OsString::from(format!("--window-size={},{}", fp.viewport_width, fp.viewport_height));
    ...
    let mut args: Vec<&OsStr> = vec![&ua, &size, &lang, &platform, &no_automation, &no_iso, &no_gpu_sandbox, &no_first_run, &no_default_check];
    if has_gpu { args.push(&enable_gpu); }

    let mut opts = LaunchOptions::default();
    opts.headless = headless;
    opts.sandbox = false;
    opts.args = args;
    if let Some(ref p) = chrome_path { opts.path = Some(p.clone().into()); }

    Browser::new(opts).map(|b| Arc::new(b)).map_err(|e| ModuleError::Runtime(format!("browser: {e}")))
}

fn strip_html(html: &str) -> String {
    let mut r = String::new(); let mut t = false;
    for c in html.chars() {
        if c == '<' { t = true; continue; } if c == '>' { t = false; continue; }
        if !t { r.push(c); }
    }
...
impl BugModule for BrowserModule {
    ...
    fn name(&self) -> &str { "浏览器模块" }
    fn version(&self) -> &str { "0.3.0" }
    fn description(&self) -> &str { "Chromium 双池 — 借鉴 BrowserContext 隔离 + --dump-dom JS注入 + 反检测" }
    async fn on_install(&self) -> Result<(), ModuleError> {
        // 自动下载 Chromium（首次使用）
        match self.get_chrome_path() {
            Ok(Some(p)) => { println!("  ✅ Chromium 就绪: {p}"); Ok(()) }
    ...
    fn capabilities(&self) -> Vec<ModuleCapability> {
        vec![ModuleCapability::WebSearch, ModuleCapability::Tool { name: "browser".into() }]
    }
    fn permissions(&self) -> Vec<ModulePermission> { vec![ModulePermission::NetworkOutbound] }
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn strip_html_works() { assert!(strip_html("<p>Hello</p>").contains("Hello")); }
    #[test] fn fingerprint_varies() {
        let a = random_fingerprint();
        // 随机性测试：生成多个确保覆盖
        let mut seen = std::collections::HashSet::new();
        for _ in 0..10 { seen.insert(random_fingerprint().user_agent); }
        assert!(seen.len() > 1, "UA未随机化");
    }
    #[test] fn pool_max_size() { assert_eq!(ContextPool::new(4, 8).max_size, 4); }
}
