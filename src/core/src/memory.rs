...
impl MemoryMesh {
    pub fn new(config: MemoryConfig) -> Self {
        Self {
            ...
            full_scenes: RwLock::new(HashMap::new()),
            config,
    ...
    pub fn init_storage(&self, home: &std::path::Path) -> Result<(), String> {
        let store = Store::open(home)?;
        if let Ok(mems) = store.load_global() {
            *self.global.write() = mems;
        }
        // 加载所有已有场景的概要
    ...
    pub fn update_summary(&self, scene_id: u64) -> Result<(), String> {
        let summaries: Vec<MemorySummary> = self
            .full_scenes
            .read()
            .get(&scene_id)
            .map(|mems| {
        ...
        if let Some(ref s) = *self.store.read() {
            s.save_summaries(scene_id, &summaries)?;
        }
        Ok(())
    }
    ...
    pub fn flush(&self) -> Result<(), String> {
        if let Some(ref s) = *self.store.read() {
            s.save_global(&self.global.read())?;
        }
        Ok(())
    }
    ...
    pub fn flush_scene(&self, scene_id: u64) -> Result<(), String> {
        if let Some(ref s) = *self.store.read() {
            if let Some(m) = self.full_scenes.read().get(&scene_id) {
                s.save_scene(scene_id, m)?;
            }
        }
    ...
    pub fn store(&self, m: Memory) {
        match &m.scope {
            MemoryScope::All => {
                self.global.write().push(m);
            }
            MemoryScope::Single(id) => {
                self.full_scenes.write().entry(*id).or_default().push(m);
            }
            _ => {}
        }
    }
    ...
    pub fn retrieve(
        ...
        if let Some(s) = self.full_scenes.read().get(&scene_id) {
            results.extend(s.iter().cloned());
        }
        results.retain(|m| {
            m.owners
                .iter()
                .any(|o| matches!(o, OwnerId::All) || o == owner)
        });
        if let Some(st) = subagent_type {
            results.retain(|m| m.subagent_type == st || m.subagent_type == SubAgentType::General);
        }
    ...
    pub fn stats(&self) -> MemoryStats {
        MemoryStats {
            ...
            total_entries: self.global.read().len()
                + self
                    .full_scenes
                    .read()
                    .values()
