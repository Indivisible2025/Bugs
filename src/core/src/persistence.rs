...
impl Store {
    pub fn open(home: &PathBuf) -> Result<Self, String> {
        ...
        let global = redb::Database::create(&data_dir.join("global.redb")).map_err(|e| e.to_string())?;
        Ok(Self { global, data_dir, limits: MemoryConfig::default() })
    }
    ...
    pub fn save_global(&self, memories: &[Memory]) -> Result<(), String> {
        ...
        {
            let mut table = write_txn.open_table(GLOBAL_TABLE).map_err(|e| e.to_string())?;
            let count = memories.len() as u64;
            { let b = count.to_be_bytes(); table.insert(COUNT_KEY, b.as_slice()).ok(); }
            for (i, m) in memories.iter().enumerate() {
                let val = serde_json::to_vec(m).map_err(|e| e.to_string())?;
                let c = zstd::encode_all(val.as_slice(), 3).map_err(|e| e.to_string())?;
                let b = (i as u64).to_be_bytes(); table.insert(b.as_slice(), c.as_slice()).ok();
            }
        }
    ...
    pub fn load_global(&self) -> Result<Vec<Memory>, String> {
        ...
        let table = read_txn.open_table(GLOBAL_TABLE).map_err(|e| e.to_string())?;
        let count: u64 = table.get(COUNT_KEY).map_err(|e| e.to_string())?
            .map(|v| u64::from_be_bytes(v.value().try_into().unwrap_or([0u8;8]))).unwrap_or(0);
        let mut memories = Vec::new();
        for i in 0..count {
            let b = i.to_be_bytes();
            if let Ok(Some(val)) = table.get(b.as_slice()) {
                let d = zstd::decode_all(val.value()).unwrap_or_else(|_| val.value().to_vec());
                if let Ok(m) = serde_json::from_slice::<Memory>(&d) { memories.push(m); }
            }
        }
    ...
    pub fn save_scene(&self, scene_id: u64, memories: &[Memory]) -> Result<(), String> {
        ...
        {
            let mut table = write_txn.open_table(SCENE_TABLE).map_err(|e| e.to_string())?;
            let count = memories.len() as u64;
            { let b = count.to_be_bytes(); table.insert(COUNT_KEY, b.as_slice()).ok(); }
            for (i, m) in memories.iter().enumerate() {
                let val = serde_json::to_vec(m).map_err(|e| e.to_string())?;
    ...
    pub fn load_scene(&self, scene_id: u64) -> Result<Vec<Memory>, String> {
        ...
        let table = read_txn.open_table(SCENE_TABLE).map_err(|e| e.to_string())?;
        let count: u64 = table.get(COUNT_KEY).map_err(|e| e.to_string())?
            .map(|v| u64::from_be_bytes(v.value().try_into().unwrap_or([0u8;8]))).unwrap_or(0);
        let mut memories = Vec::new();
        for i in 0..count {
    ...
    fn open_scene_db(&self, scene_id: u64) -> Result<redb::Database, String> {
        let p = self.data_dir.join("scenes").join(format!("{scene_id}.redb"));
    ...
    pub fn save_summaries(&self, scene_id: u64, summaries: &[MemorySummary]) -> Result<(), String> {
        ...
        let write_txn = db.begin_write().map_err(|e| e.to_string())?;
        {
            let mut table = write_txn.open_table(SUMMARY_TABLE).map_err(|e| e.to_string())?;
            let count = summaries.len() as u64;
            { let b = count.to_be_bytes(); table.insert(COUNT_KEY, b.as_slice()).ok(); }
            for (i, s) in summaries.iter().enumerate() {
                let val = serde_json::to_vec(s).map_err(|e| e.to_string())?;
                let b = (i as u64).to_be_bytes(); table.insert(b.as_slice(), val.as_slice()).ok();
            }
        }
        write_txn.commit().map_err(|e| e.to_string())?;
        Ok(())
    ...
    pub fn load_summaries(&self, scene_id: u64) -> Result<Vec<MemorySummary>, String> {
        let db = self.open_scene_db(scene_id)?;
        let read_txn = db.begin_read().map_err(|e| e.to_string())?;
        let table = read_txn.open_table(SUMMARY_TABLE).map_err(|e| e.to_string())?;
        let count: u64 = table.get(COUNT_KEY).map_err(|e| e.to_string())?
            .map(|v| u64::from_be_bytes(v.value().try_into().unwrap_or([0u8;8]))).unwrap_or(0);
        ...
        for i in 0..count {
            let b = i.to_be_bytes();
            if let Ok(Some(val)) = table.get(b.as_slice()) {
                if let Ok(s) = serde_json::from_slice::<MemorySummary>(val.value()) { summaries.push(s); }
            }
        }
        Ok(summaries)
    }
    ...
    pub fn save_state(&self, config: &BugsConfig) -> Result<(), String> {
        ...
        std::fs::write(&tmp, &serde_json::to_string_pretty(config).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;
...
mod tests {
    use super::*;
    fn s() -> (Store, tempfile::TempDir) { let d=tempfile::TempDir::new().unwrap(); (Store::open(&d.path().to_path_buf()).unwrap(), d) }
    fn m() -> Memory { Memory { agent_id:1, scene_id:1, scope:MemoryScope::All, owners:vec![OwnerId::All], category:MemoryCategory::Knowledge, subagent_type:SubAgentType::General, enhancement_rate:0.5, decay_rate:0.1, strength_cached:8.0, last_updated:0, last_validated:0, validation_count:3, source_count:2, seq:1 } }

    #[test] fn global_rt() { let (s,_)=s(); s.save_global(&[m()]).unwrap(); assert_eq!(s.load_global().unwrap().len(),1); }
    #[test] fn scene_rt() { let (s,_)=s(); s.save_scene(1,&[m()]).unwrap(); assert_eq!(s.load_scene(1).unwrap().len(),1); }
    #[test] fn zstd_compresses() { let (s,_)=s(); s.save_global(&[m()]).unwrap(); let l=s.load_global().unwrap(); assert!((l[0].strength_cached-8.0).abs()<0.01); }
}
