...
impl ToolRegistry {
    pub fn new() -> Self {
        ...
        registry.register(ToolDefinition {
            name: "exec".into(),
            group: "group:runtime".into(),
            description: "执行命令".into(),
            required: true,
        });
        registry.register(ToolDefinition {
            name: "memory_search".into(),
            group: "group:memory".into(),
        ...
        registry.register(ToolDefinition {
            ...
            group: "group:memory".into(),
            description: "写入记忆".into(),
            required: true,
        });
        registry.register(ToolDefinition {
            name: "knowledge_query".into(),
            group: "group:knowledge".into(),
            description: "知识查询".into(),
            required: true,
        });
        ...
        registry.register(ToolDefinition {
            name: "browser".into(),
            group: "group:browser".into(),
            description: "浏览器搜索".into(),
            required: false,
        });
        registry.register(ToolDefinition {
            name: "code_exec".into(),
            group: "group:sandbox".into(),
            description: "代码沙箱执行".into(),
            required: false,
        });
        ...
        if let Some(tool) = self.tools.get(name) {
            if tool.required {
                return !config
                    .deny
                    .iter()
                    .any(|d| d == &tool.group || d == &tool.name);
            }
            return config
                .allow
                .iter()
                .any(|a| a == &tool.group || a == &tool.name);
        }
        false
    }
