...
fn var(k: &str) -> Result<String, std::env::VarError> {
    std::env::var(k)
}
...
fn auto_register_from_env() -> (String, Vec<String>) {
    ...
    if var("OPENAI_API_KEY").is_ok() {
        providers.push("openai".into());
    }
    if var("ANTHROPIC_API_KEY").is_ok() {
        providers.push("anthropic".into());
    }
    if var("DEEPSEEK_API_KEY").is_ok() {
        providers.push("deepseek".into());
    }
    if var("OLLAMA_BASE_URL").is_ok() {
...
async fn main() -> io::Result<()> {
    ...
    if daemon_alive {
        ...
        app.add_msg(
            MsgRole::System,
            format!("⚠️ Daemon未运行。直接模式。模型: {model}"),
        );
                ...
                Event::Key(key) if key.kind == KeyEventKind::Press => {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            ...
                            app.cursor = 0;
                            if input == "/exit" {
                                app.add_msg(MsgRole::System, app.i18n.exit_msg().into());
                                break;
                            }
                            if input == "/clear" {
                                app.clear_chat();
                            ...
                            if input.starts_with("/scene")
                                || input.starts_with("/memory")
