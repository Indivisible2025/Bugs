...
pub fn draw(f: &mut Frame, app: &App) {
    ...
    // ── 标签页 ──
    let tab_names = vec![" Chat ", " Scenes ", " Trust ", " Status "]
        .into_iter()
...
fn draw_chat(f: &mut Frame, app: &App, area: Rect) {
    ...
    let visible_msgs: Vec<&ChatMsg> = app
        .messages
        .iter()
        .rev()
        .skip(app.scroll)
    ...
    let input = Paragraph::new(input_text)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(app.i18n.input_placeholder()),
        )
...
fn draw_scenes(f: &mut Frame, _app: &App, area: Rect) {
    ...
    for (name, current) in &_app.scenes {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled(
                name,
                Style::default().fg(if *current { Color::Green } else { Color::White }),
            ),
        ]));
    }
    ...
    let p = Paragraph::new(Text::from(lines))
        .block(Block::default().borders(Borders::ALL).title("📋 场景"));
...
fn draw_trust(f: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(Line::from(Span::raw("")));
    lines.push(Line::from(Span::raw(
        " 这些记忆信任值偏低，可运行 /meditate 整理",
    )));
    let p = Paragraph::new(Text::from(lines))
        .block(Block::default().borders(Borders::ALL).title("🔐 信任引擎"));
    f.render_widget(p, area);
}
